...
# ....
#
  if ARGV.size == 0 or ARGV.size != 8
    print "USAGE: script.rb <listf> <speAgff> <speAfas> <speBgff> <speAfas> <length_upstream> <max_gene_length> <outdir>\n"
    exit
  end
...
#    STDERR.puts speAgffh[ speAgid ]
    ...
    speA_regst, speA_regen = ClipUpstream.calc_start(speA_gstr, speA_scln, length_upst, speA_gstt, speA_gend, max_gene_length )
    speB_regst, speB_regen = ClipUpstream.calc_start(speB_gstr, speB_scln, length_upst, speB_gstt, speB_gend, max_gene_length)
  
    speA_genes = ClipUpstream.get_genes_in_region( speAscagffh, speAgffh, speA_scID, speA_regst, speA_regen )
    speB_genes = ClipUpstream.get_genes_in_region( speBscagffh, speBgffh, speB_scID, speB_regst, speB_regen )
    ...
    speA_annotation_lines_set = ClipUpstream.get_vista_annotation( speA_gstr, speAgffh, speA_genes, speA_regst, speA_regen )
    speA_annotation_lines_set.each do |speA_annotation_lines| 
      speA_annotation_lines.each{ |line| speAoutan.print line }
    end

    speB_annotation_lines_set = ClipUpstream.get_vista_annotation( speB_gstr, speBgffh, speB_genes, speB_regst, speB_regen )
